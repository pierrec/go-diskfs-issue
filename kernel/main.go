...
func _start() {
	...
	if framebufferRequest.Response == nil || len(framebufferRequest.Response.Framebuffers()) < 1 {
		...
		const attr = 0<<4 | 0xf
		fb := unsafe.Slice((*uint16)(unsafe.Pointer(fbPhysAddr)), 80*25)
		fb[0] = uint16(attr)<<8 | uint16('X')
		hcf()
	}
	frame_buffers := framebufferRequest.Response.Framebuffers()
	// Fetch the first framebuffer.
	framebuffer := frame_buffers[0]
